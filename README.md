...
并在 `FixedUpdate` 中按链式顺序执行：

1. `update_active_effect_tag_requirements_system`
2. `(tick_effect_duration_system, tick_effect_period_system)`
3. `tick_ability_tasks_system`
4. `process_gameplay_effect_application_queue_system`
5. `process_ability_activation_queue_system`
6. `cleanup_finished_abilities_system`
7. `recalculate_attribute_sets_system`

这个顺序决定了每个 fixed tick 的运行语义：

```text
已有 ActiveEffect 更新状态
  -> duration / period 推进
  -> AbilityTask 推进
  -> Task 产生的 Effect 请求被消费
  -> Task 产生的 Ability 激活请求被消费
  -> 结束或取消的 ActiveAbility 被清理
  -> AttributeSet 统一重算兜底
```
...
3. 调用 `can_activate_ability`
4. 根据 ability 的 `cancel_abilities_with_tags` 取消其它 active ability
5. 调用 `start_ability`
6. 调用 `commit_ability`
7. commit 失败则 rollback
8. commit 成功后应用 activation effects
9. spawn startup tasks
10. 如果 `end_on_activation` 为 true，把 active ability 标记为 `Ending`
...
- Attribute 读取采用懒重算。
- `recalculate_attribute_sets_system` 仍保留在 FixedUpdate 末尾，作为兜底重算。
...
- ActiveEffect 作为 target 子实体存在，但清理 modifier/tag 依赖显式 cleanup。
...
- `recalculate_attribute_sets_system` 在懒重算后主要是兜底，可以根据性能需求决定是否保留。
- `AbilityTaskDef` 类型还较少，可以扩展 `WaitEvent`、`WaitCollision`、`SpawnProjectile`、`SpawnArea` 等任务。
- `AbilityTaskOnFinishedDef` 目前没有暴露 `ActivateAbility`，虽然运行期 enum 已支持。
